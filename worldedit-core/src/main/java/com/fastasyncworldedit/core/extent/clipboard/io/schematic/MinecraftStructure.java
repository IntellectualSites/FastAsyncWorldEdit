...
public class MinecraftStructure implements ClipboardReader, ClipboardWriter {
    ...
    public void write(Clipboard clipboard, String owner) throws IOException {
        ...
        Map<String, Object> structure = FaweCache.faweCache().asMap("version", 1, "author", owner);
        ...
        for (BlockVector3 point : region) {
            ...
            if (block.getBlockType() != BlockTypes.STRUCTURE_VOID) {
                ...
                if (!block.hasNbtData()) {
                    blocks.add(FaweCache.faweCache().asMap("state", index, "pos", pos));
                } else {
                    blocks.add(
                            FaweCache.faweCache().asMap("state", index, "pos", pos, "nbt", block.getNbtData()));
                }
            }
        }
        if (!blocks.isEmpty()) {
        ...
        for (Entity entity : clipboard.getEntities()) {
            ...
            if (state != null) {
                ...
                Map<String, Object> entityMap = FaweCache.faweCache().asMap("pos", pos, "blockPos", blockPos, "nbt", nbt);
        ...
        out.writeNamedTag("", FaweCache.faweCache().asTag(structure));
        close();
    }
