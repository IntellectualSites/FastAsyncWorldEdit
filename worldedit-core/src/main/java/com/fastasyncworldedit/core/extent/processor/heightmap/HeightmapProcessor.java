...
public class HeightmapProcessor implements IBatchProcessor {
    ...
    private static final char[] AIR_LAYER_CHAR = new char[4096];
    private static final int[] AIR_LAYER_INT = new int[4096];

    static {
        Arrays.fill(COMPLETE, true);
        Arrays.fill(AIR_LAYER_CHAR, (char) BlockTypesCache.ReservedIDs.AIR);
        Arrays.fill(AIR_LAYER_INT, BlockTypesCache.ReservedIDs.AIR);
    }
