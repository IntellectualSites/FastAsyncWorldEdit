...
public class NukkitPlayer extends AbstractPlayerActor {
    ...
    public NukkitPlayer(Player player) {
        this.player = player;
        // Hydrate the clipboard from disk on login, matching the Bukkit adapter. Guard against the
        // FAWE platform not being initialised (e.g. in unit tests) so construction never throws.
        if (com.fastasyncworldedit.core.Fawe.platform() != null
                && com.fastasyncworldedit.core.configuration.Settings.settings().CLIPBOARD.USE_DISK) {
            try {
                loadClipboardFromDisk();
            } catch (RuntimeException ignored) {
                // Best-effort: clipboard hydration must not break player construction.
            }
        }
    }
    ...
    public Locale getLocale() {
        try {
            String code = NukkitImplLoader.get().getPlayerLanguageCode(player);
            if (code == null) {
    ...
    public <B extends BlockStateHolder<B>> void sendFakeBlock(BlockVector3 pos, B block) {
        ...
        if (block == null) {
            ...
            pk.blockRuntimeId = adapter.getBlockRuntimeId(player, fullId);
    ...
    public void sendTitle(Component title, Component sub) {
        // Apply locale-aware translation/formatting before serializing, matching print().
        String titleStr = PlainComponentSerializer.INSTANCE.serialize(
                WorldEditText.format(title, getLocale())
        );
        String subStr = PlainComponentSerializer.INSTANCE.serialize(
                WorldEditText.format(sub, getLocale())
        );
