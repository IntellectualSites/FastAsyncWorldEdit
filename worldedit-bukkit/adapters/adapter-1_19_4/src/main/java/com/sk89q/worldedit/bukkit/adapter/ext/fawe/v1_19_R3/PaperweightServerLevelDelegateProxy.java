...
public class PaperweightServerLevelDelegateProxy implements InvocationHandler {
    ...
    private boolean setBlock(BlockPos blockPos, BlockState blockState) {
        try {
            return editSession.setBlock(
                    BlockVector3.at(blockPos.getX(), blockPos.getY(), blockPos.getZ()),
                    adapter.adapt(blockState)
            );
    ...
    private boolean addEntity(Entity entity) {
        ...
        BaseEntity baseEntity = new BaseEntity(
                ...
                (com.sk89q.jnbt.CompoundTag) adapter.toNative(tag)
        );
