...
public class FaweDelegateRegionManager {
    ...
    public boolean handleClear(@NotNull Plot plot,
        ...
        TaskManager.IMP.async(() -> {
            synchronized (FaweDelegateRegionManager.class) {
                ...
                EditSession editSession = WorldEdit.getInstance().newEditSessionBuilder().world(world).checkMemory(false).fastmode(true).limitUnlimited().changeSetNull().autoQueue(false).build();
                ...
                if (hybridPlotWorld.PLOT_SCHEMATIC) {
                    ...
                    EditSession scheditsession = !Settings.Schematics.PASTE_ON_TOP ? editSession :
                        WorldEdit.getInstance().newEditSessionBuilder().world(world).checkMemory(false).fastmode(true).limitUnlimited().changeSetNull().autoQueue(false).build();
    ...
    public void setBiome(CuboidRegion region, int extendBiome, BiomeType biome, String world, Runnable whenDone) {
        ...
        TaskManager.IMP.async(() -> {
            synchronized (FaweDelegateRegionManager.class) {
                EditSession editSession = WorldEdit.getInstance().newEditSessionBuilder().world(BukkitAdapter.adapt(getWorld(world))).checkMemory(false).fastmode(true).limitUnlimited().changeSetNull().autoQueue(false).build();
    ...
    public boolean regenerateRegion(final Location pos1, final Location pos2, boolean ignore, final Runnable whenDone) {
        TaskManager.IMP.async(() -> {
            synchronized (FaweDelegateRegionManager.class) {
                World pos1World = BukkitAdapter.adapt(getWorld(pos1.getWorldName()));
                try (EditSession editSession = WorldEdit.getInstance().newEditSessionBuilder().world(pos1World).checkMemory(false).fastmode(true).limitUnlimited().changeSetNull().autoQueue(false).build()) {
                    CuboidRegion region = new CuboidRegion(BlockVector3.at(pos1.getX(), pos1.getY(), pos1.getZ()), BlockVector3.at(pos2.getX(), pos2.getY(), pos2.getZ()));
